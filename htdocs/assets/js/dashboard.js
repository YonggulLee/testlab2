window.addEventListener('DOMContentLoaded', async function () {
	...
	// 기상대
	document.querySelector("#weather-temp").innerText = Number(weather.outTemp[0].value).toFixed(2);
	document.querySelector("#weather-humid").innerText = Number(weather.outHumidity[0].value).toFixed(1);
	document.querySelector("#weather-pressure").innerText = Number(weather.pressure[0].value).toFixed(2);
	document.querySelector("#weather-uv").innerText = Number(weather.uv[0].value).toFixed(2);
	document.querySelector("#weather-rain-day").innerText = Number(weather.dayRain[0].value).toFixed(2);
	document.querySelector("#weather-rain-hour").innerText = Number(weather.hourRain[0].value).toFixed(2);
	document.querySelector("#weather-rad").innerText = Number(weather.radiation[0].value).toFixed(0);
	document.querySelector("#weather-windspeed").innerText = Number(weather.windSpeed[0].value).toFixed(2);

	// 스프링클러
	document.querySelector("#sprinkler-1").innerText = sprinkler.switch1[0].value == "1" ? "켜짐" : "꺼짐";
	document.querySelector("#sprinkler-2").innerText = sprinkler.switch2[0].value == "1" ? "켜짐" : "꺼짐";
	document.querySelector("#sprinkler-3").innerText = sprinkler.switch3[0].value == "1" ? "켜짐" : "꺼짐";
	document.querySelector("#sprinkler-4").innerText = sprinkler.switch4[0].value == "1" ? "켜짐" : "꺼짐";
	document.querySelector("#sprinkler-5").innerText = sprinkler.switch5[0].value == "1" ? "켜짐" : "꺼짐";
	document.querySelector("#sprinkler-6").innerText = sprinkler.switch6[0].value == "1" ? "켜짐" : "꺼짐";
	...
	// Chart drawing
	{
		const [{ data: noise }, { data: dust }, { data: vibe }] = await Promise.all([
			farota.get(URI_NOISE, { params: { startTs, endTs, limits: 1000, keys: 'leq,lmax' } }),
			farota.get(URI_DUST, { params: { startTs, endTs, limits: 1000, keys: 'finedust,ultraFinedust' } }),
			farota.get(URI_VIBE, { params: { startTs, endTs, limits: 1000, keys: 'x_1,y_1,z_1,x_2,y_2,z_2' } })
		])
	...
	// RPC Call
	{
		const sprinklerCtrl = async (valveNo, onOff) => {
			...
			await farota.post('/plugins/rpc/oneway/446e8620-b923-11eb-a7c7-c5ac42947b75', body)

			document.querySelector('#sprinkler-' + valveNo).innerText = onOff == '1' ? '켜짐' : '꺼짐'
		}

		document.querySelector('#ctrl-1 .sprinkler-btn-on').addEventListener('click', () => { sprinklerCtrl(1, true) })
		document.querySelector('#ctrl-1 .sprinkler-btn-off').addEventListener('click', () => { sprinklerCtrl(1, false) })
		document.querySelector('#ctrl-2 .sprinkler-btn-on').addEventListener('click', () => { sprinklerCtrl(2, true) })
		document.querySelector('#ctrl-2 .sprinkler-btn-off').addEventListener('click', () => { sprinklerCtrl(2, false) })
		document.querySelector('#ctrl-3 .sprinkler-btn-on').addEventListener('click', () => { sprinklerCtrl(3, true) })
		document.querySelector('#ctrl-3 .sprinkler-btn-off').addEventListener('click', () => { sprinklerCtrl(3, false) })
		document.querySelector('#ctrl-4 .sprinkler-btn-on').addEventListener('click', () => { sprinklerCtrl(4, true) })
		document.querySelector('#ctrl-4 .sprinkler-btn-off').addEventListener('click', () => { sprinklerCtrl(4, false) })
		document.querySelector('#ctrl-5 .sprinkler-btn-on').addEventListener('click', () => { sprinklerCtrl(5, true) })
		document.querySelector('#ctrl-5 .sprinkler-btn-off').addEventListener('click', () => { sprinklerCtrl(5, false) })
		document.querySelector('#ctrl-6 .sprinkler-btn-on').addEventListener('click', () => { sprinklerCtrl(6, true) })
		document.querySelector('#ctrl-6 .sprinkler-btn-off').addEventListener('click', () => { sprinklerCtrl(6, false) })
	}
